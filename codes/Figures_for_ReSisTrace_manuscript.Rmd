...
```
#### source data table and statistic table
...
stat <- sister_d$distance %>%
  group_by(group) %>%
  summarise(mean = mean(similarity, na.rm = TRUE),
            sd = sd(similarity, na.rm = TRUE),
            n = n()) %>%
  mutate(se = sd / sqrt(n),
         lower.ci = mean - qt(1 - (0.05 / 2), n - 1) * se,
         upper.ci = mean + qt(1 - (0.05 / 2), n - 1) * se)
save(test, stat, file = paste0(sub_output_dir, "sister_distances_stat_test.RData"))
```

```{r}
sister_d2 <- sister_d$distance %>% 
  group_by(group) %>%
  mutate(row = row_number()) %>%
  tidyr::pivot_wider(names_from = group, values_from = similarity) %>%
  select(-row)
```

```{r}
save(sister_d2, file = paste0(sub_output_dir, "sister_distances_plot_table.RData"))
```
...
write_xlsx(data_plot, paste0(sub_output_dir, "Figure1E_conserve_gene_logFC_p_plot_table.xlsx"))
save(cor, file = paste0(sub_output_dir, "Figure1E_conserve_gene_logFC_p_stat_test.RData"))
```
...
p <- 
  ...
  guides(colour = "none")
p
ggsave(paste0(sub_output_dir, "Figure1E_conserve_gene_adjust_p.pdf"), p,
       width = 4, height = 4)
```
...
ggsave(
  paste0(
    sub_output_dir,
    "Figure1F_sister_conserve_gene_dynamic_log_scale_second_bin.pdf"
  ),
...
write_xlsx(t, paste0(
    sub_output_dir,
    "Figure1F_sister_conserve_gene_dynamic_log_scale_second_bin_plot_table.xlsx"
  ))
```
...
write_xlsx(plot_table, paste0(sub_output_dir, "Figure1G_non_conserved_KEGG_volcano_plot_table.xlsx"))
...
p
ggsave(paste0(sub_output_dir, "Figure1G_non_conserved_KEGG_volcano.pdf"), p,
       width = 4, height = 4)
```
...
write.csv(plot_table, paste0(sub_output_dir, "FigureS1A_proportion_of_cells_having_label_plot_table.csv"), row.names = FALSE)
...
ggsave(paste0(sub_output_dir, "FigureS1A_proportion_of_cells_having_label_preTreatment.pdf"), width = 8, height = 5.3)
...
ggsave(paste0(sub_output_dir, "FigureS1A_proportion_of_cells_having_label_postTreatment.pdf"), width = 8, height = 5.3)
```
...
write.csv(plot_table, paste0(sub_output_dir, "FigureS1B_unique_label_per_bt_sample_plot_table.csv"), row.names = FALSE)
...
ggsave(paste0(sub_output_dir, "FigureS1B_unique_label_per_bt_sample.pdf"), width = 7.5, height = 5)
...
dot_data <- data %>% 
  mutate(
    replicate = rep(rep(c(1:3), each = 3), times = 6),
    accumulate_prop = 0
  )

for (i in seq(1, nrow(dot_data) - 2, by = 3)) {
  #G2/M
  dot_data$accumulate_prop[i+2] <- dot_data$proportion[i+2]
  #G1
  dot_data$accumulate_prop[i] <- plot_table$SDPos[plot_table$group == dot_data$group[i] & plot_table$cell_cycle == "G2/M phase"] + dot_data$proportion[i]
  #S
  dot_data$accumulate_prop[i+1] <- plot_table$SDPos[plot_table$group == dot_data$group[i] & plot_table$cell_cycle == "G1 phase"] + dot_data$proportion[i+1]
...
p <- ggplot(plot_table,
  ...
  geom_point(
    data = dot_data,
    mapping = aes(x = group, y = accumulate_prop),
    color = "grey",
    fill = "grey",
    position = position_jitter(width = 0.4, seed = 1)
  ) +
  geom_errorbar(
    aes(ymin = SDPos - proportion_sd, ymax = SDPos + proportion_sd),
    width = 0.3,
    position = "identity"
  ) +
  theme_classic() +
...
write.csv(plot_table, paste0(sub_output_dir, "FigureS1F_cell_division_times.csv"), row.names = FALSE)
...
p <- ggplot(plot_table, aes(x = Division, y = mean, fill = Division)) +
  ...
  geom_point(
    data = data,
    mapping = aes(x = Division, y = Proportion),
...
ggsave(
  paste0(sub_output_dir, "FigureS1F_cell_division_times.pdf"),
  width = 5, height = 4.8
)
```
...
write.csv(
  plot_table,
  paste0(sub_output_dir, "FigureS1G_frequence_of_lineage_label.csv"),
  row.names = FALSE
)
```
...
ggsave(
  paste0(sub_output_dir, "FigureS1G_frequence_of_lineage_label.pdf"),
  width = 5, height = 4.8
)
```
...
## J: preR-preS DEGs correlation between replicates
...
for (t in treatments){
  ...
  # Plot table
  write_xlsx(
    select(deg, "ensembl", paste0(t_short, "_1_avg_log2FC"), paste0(t_short, "_2_avg_log2FC")),
    paste0(sub_output_dir, "FigureS1J_correlation_between_replicates_", t, "_plot_table.xlsx")
  )
  ...
  save(cor, file = paste0(sub_output_dir, "FigureS1J_correlation_between_replicates_", t, "_stat_test.RData"))
  range <- range(
    c(
      deg[[paste0(t_short, "_1_avg_log2FC")]],
      deg[[paste0(t_short, "_2_avg_log2FC")]]
  ...
  p
  ggsave(paste0(sub_output_dir, "FigureS1J_correlation_between_replicates_", t, ".pdf"), p,
         width = 5, height = 5)
}
...
save(t_test, file = paste0(sub_output_dir, "FigureS2A_sister_conserve_gene_exp_distribution_stat_test.RData"))
...
write_xlsx(
  plot_table,
  paste0(sub_output_dir, "FigureS2A_sister_conserve_gene_exp_distribution.xlsx")
)
```
...
ggsave(
  paste0(sub_output_dir, "FigureS2B_Degradation_Transcription_second_bin.pdf"), 
  plot = p, width = 8, height = 5.5
)
write_xlsx(plot_table, paste0(sub_output_dir, "FigureS2B_Degradation_Transcription_second_bin_plot_table.xlsx"))

t_test <- t.test(plot_table$Degradation_Transcription[plot_table$conserve_gene == "Sister-concordant\ngene"], plot_table$Degradation_Transcription[plot_table$conserve_gene == "Sister-discordant\ngene"])
save(t_test, file = paste0(sub_output_dir,  "FigureS2B_Degradation_Transcription_second_bin_stat_test.RData"))
t_test
```
...
ggsave(
  paste0(sub_output_dir, "FigureS2C_Splicing_Transcription_second_bin.pdf"), 
  plot = p, width = 8, height = 5.5
)
write_xlsx(plot_table, paste0(sub_output_dir, "FigureS2C_Splicing_Transcription_second_bin_plot_table.xlsx"))

t_test <- t.test(plot_table$Splicing_Transcription[plot_table$conserve_gene == "Sister-concordant\ngene"], plot_table$Splicing_Transcription[plot_table$conserve_gene == "Sister-discordant\ngene"])
save(t_test, file = paste0(sub_output_dir,  "FigureS2C_Splicing_Transcription_second_bin_stat_test.RData"))
t_test
```
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_preS.pdf"), width = 4, height = 4)
write_xlsx(res2, paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_preS_plot_table.xlsx"))
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_preR.pdf"), width = 4, height = 4)
write_xlsx(res3, paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_preR_plot_table.xlsx"))
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_postT.pdf"), width = 4, height = 4)
write_xlsx(res4, paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_postT_plot_table.xlsx"))
```
...
for (i in LETTERS[1:5]){
  ...
  for (t in c("Carboplatin", "Olaparib", "NK")){
    ...
    save(p_rs, file = paste0(sub_output_dir, "Figure2C_preR_vs_preS_propoetion_test_table_", t, "_subclone_", i, ".RData"))
...
#### Downsampling in each treatment-drugSens group (Revision 2)
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_preR_downsampling_bar_plot.pdf"), width = 6, height = 4)
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_preS_downsampling_bar_plot.pdf"), width = 6, height = 4)
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_posT_downsampling_bar_plot.pdf"), width = 6, height = 4)
```
...
for (i in c("A", "B", "C", "D", "E")){
  ...
  ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_subclone_", i, "_downsampling_bar_plot.pdf"), width = 7, height = 4)
}
save(pvalue, file = paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_subclone_specific_downsampling_bar_plot_pvalues.RData"))
```
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_preR_downsampling.pdf"), width = 4, height = 4)
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_preS_downsampling.pdf"), width = 4, height = 4)
...
ggsave(paste0(sub_output_dir, "Figure2C_cell_proportion_CNV_posT_downsampling.pdf"), width = 4, height = 4)
```
...
Connectivity score using PreR-preS original P value
...
sink(paste0(sub_output_dir, "FigureS3C_shannon.txt"))
res = list()
k = 1
...
```{r}
ks_test <- ks.test(tmp$Freq[tmp$time_point == "pre-treatment"], tmp$Freq[tmp$time_point == "post-treatment"], exact = TRUE)
save(ks_test, file = paste0(sub_output_dir, "FigureS3B_cumulate_all.RData"))
ks_test
```
...
write.csv(tmp, paste0(sub_output_dir, "FigureS3B_accumulated_shannon.csv"), row.names = FALSE)
...
ggsave(
  paste0(sub_output_dir, "FigureS3B_cumulate_all.pdf"),
...
```{r}
shannon = read.table(paste0(sub_output_dir, "FigureS3C_shannon.txt"))
...
write.csv(shannon, paste0(sub_output_dir, "FigureS3C_shannon_plot_table.csv"), row.names = FALSE)
values1 = c(blue, red, brown, green)
...
p <- ggplot(shannon, aes(x = time_point, y = value)) +
  ...
  scale_color_manual(values = values1)
ggsave(
  paste0(sub_output_dir, "FiguerS3C_shannon.pdf"),
  p,
  width = 9, height = 5
  )
...
save(t_test, file = paste0(sub_output_dir, "FigureS3C_shannon_stat_test.RData"))
t_test
```
...
stat.test <- plot_table %>%
  group_by(group) %>% 
  t_test(RAD51 ~ cell, detailed = TRUE) %>%
  add_xy_position(
...
write.csv(
  as.data.frame(select(stat.test, -groups)),
  paste0(sub_output_dir, "Figure3C_COV362_HRD_bar_stat_test.csv"),
  row.names = FALSE
)
...
p <- ggplot(my_sum, aes(x = as.factor(cell), y = mean)) +
  geom_bar(
    aes(fill = cell),
    alpha = 1,
    position=position_dodge(),
  ...
  geom_point(
    data = plot_table,
    aes(x = as.factor(cell), y = RAD51, fill = cell),
    size = 2,
    shape = 19,
    alpha = 1,
    color = "black",
  ...
  geom_errorbar(
    aes(ymin = mean - sd, ymax = mean + sd, group = cell),
    color = "black",
    linewidth = 0.8,
    width=.5,
    position = position_dodge(.9)
...
write.csv(plot_table, paste0(sub_output_dir, "Figure3C_COV362_HRD_bar_plot_table.csv"), row.names = FALSE)
```
...
stat.test <- plot_table %>%
  group_by(Olaparib) %>%
  t_test(Viability ~ cell, detailed = TRUE) %>%
  add_xy_position(
...
write.csv(
  as.data.frame(select(stat.test, -groups)),
  paste0(sub_output_dir, "Figure3D_COV362_Olaparib_line_plot_stat_test.csv"),
  row.names = FALSE
)
...
ggsave(
  paste0(sub_output_dir, "Figure3D_COV362_Olaparib_line_plot.pdf"),
  width = 7.2, height = 3
)
ggsave(
  paste0(sub_output_dir, "Figure3D_COV362_Olaparib_line_plot.png"),
  width = 7, height = 3.5
)
write.csv(
  plot_table,
  paste0(sub_output_dir, "Figure3D_COV362_Olaparib_line_plot_plot_table.csv"),
  row.names = FALSE
)
```
...
Weighted by PreR-preS non-adjusted P value or HRD non-adjusted P value
...
stat.test <- res3.df %>%
  t_test(value ~ treatment, ref.group = "Control", detailed = TRUE) %>%
  add_xy_position(
...
write.csv(
  as.data.frame(select(stat.test, -groups)),
    paste0(
    sub_output_dir,
    "Figure3E_HRD_origin_p_preRpreS_gwc_connect_cov_weighted_spearman_p_value_stat_test_",
...
write.csv(
  ...
  paste0(
    sub_output_dir,
    "Figure3E_HRD_origin_p_preRpreS_gwc_connect_cov_weighted_spearman_p_value_plot_table",
    ifelse(conserve_gene_only, "_conserve_gene.csv", ".csv")
  )
)
```
...
for (i in 1:nrow(data_area)){
  ...
  stat.test <- table %>%
    group_by(rate) %>%
    t_test(cell_viability ~ sample, detailed = TRUE) %>%
    add_xy_position(
  ...
  write.csv(
    as.data.frame(select(stat.test, -groups)),
     paste0(sub_output_dir, gsub("\\ \\+.*", "", data_area$control[i]), "_donor_", data_area$donor[i], "_NK_bar_plot_stat_test.csv"),
    row.names = FALSE
  )
  
  my_sum <- table %>%
  group_by(rate, sample) %>%
  summarise( 
    n=n(),
  ...
  # bar plot with dot
  top_y <- max(c(my_sum$mean + my_sum$sd), table$cell_viability + 1.5)
  if (top_y < 110) {
    top_y <- 110
  }
  p <- ggplot(my_sum, aes(x = as.factor(rate), y = mean)) +
    geom_bar(
      aes(fill = sample),
      alpha = 0.6,
      position=position_dodge(),
    ...
    geom_point(
      data = table,
      aes(x = as.factor(rate), y = cell_viability, color = sample),
      size = 2,
      shape = 19,
      alpha = 1,
      # color = "black",
    ...
    scale_color_manual(values = c("#c94232FF", "#3C5488FF")) +
    geom_errorbar(
      aes(ymin = mean - sd, ymax = mean + sd, group = sample),
      color = "black",
      linewidth = 0.8,
      width=.5,
      position = position_dodge(.9)
    ...
    scale_y_continuous(limits = c(0, top_y), breaks=seq(0, 100, 25), expand = c(0, 0)) +
    theme_classic() +
    theme(
      legend.position = "right",
      legend.text = element_text(size = 21),
  ...
  ggsave(
    paste0(sub_output_dir, gsub("\\ \\+.*", "", data_area$control[i]), "_donor_", data_area$donor[i], "_NK_bar_dot_plot.pdf"),
    width = 7.5, height = 3.5
  )
  write.csv(table, paste0(sub_output_dir, gsub("\\ \\+.*", "", data_area$control[i]), "_donor_", data_area$donor[i], "_NK_bar_dot_plot_plot_table.csv"))
}
```
...
for (s in samples) {
  ...
  write.csv(
    links,
    paste0(sub_output_dir, "FigureS4B_CNV_subclone_transfer_", s, "_plot_table.csv"),
    row.names = FALSE)
}
...
data_plot %>% 
  na.omit() %>% 
  mutate(HRD = ifelse(group ==0, "high", "low")) %>% 
  write.csv(
    paste0(sub_output_dir,"FigureS5A_threshold_", p_threshold, ifelse(conserve_gene_only, "BRCA_COV_OS_conserve_gene_plot_table.csv", "BRCA_COV_OS_plot_table.csv")),
    row.names = FALSE
  )
```
...
write.csv(
  as.data.frame(select(stat.test, -groups)),
  paste0(sub_output_dir, "FigureS5E_Kura_HRD_bar_stat_test.csv"),
  row.names = FALSE
)
...
ggsave(
  paste0(sub_output_dir, "FigureS5E_Kura_HRD_bar.pdf"),
  p,
...
write.csv(
  plot_table,
  paste0(sub_output_dir, "FigureS5E_Kura_HRD_bar_plot_table.csv"),
  row.names = FALSE
)
```
...
write.csv(
  as.data.frame(select(stat.test, -groups)),
  paste0(sub_output_dir, "FigureS5F_UWB_HRD_bar_stat_test.csv"),
  row.names = FALSE
)
...
ggsave(
  paste0(sub_output_dir, "FigureS5F_UWB_HRD_bar.pdf"),
  p,
  width = 7, height = 2.5
)
...
write.csv(
  plot_table,
  paste0(sub_output_dir, "FigureS5F_UWB_HRD_bar_plot_table.csv"),
  row.names = FALSE
)
```
...
write.csv(
  as.data.frame(select(stat.test, -groups)),
  paste0(sub_output_dir, "FigureS5G_UWB1.289_Olaparib_line_plot_stat_test.csv"),
  row.names = FALSE
)
...
ggsave(
  paste0(sub_output_dir, "FigureS5G_UWB1.289_Olaparib_line_plot.pdf"),
    width = 7.2, height = 3
)
ggsave(
  paste0(sub_output_dir, "FigureS5G_UWB1.289_Olaparib_line_plot.png"),
  width = 7.3, height = 3.5
)

write.csv(
  plot_table,
  paste0(sub_output_dir, "FigureS5G_UWB1.289_Olaparib_line_plot_plot_table.csv"),
  row.names = FALSE
)
```
...
for (t in treatments){
  ...
  save(
    cor,
    file = paste0(sub_output_dir, "correlation_between_replicates_", t, "_stat_test.RData")
  )
  ...
  if (is.null(deg_merge)){
    deg_merge <- deg %>% select(
    ensembl,
    paste0(gsub(" ", "_", t), "_1_avg_log2FC"),
    paste0(gsub(" ", "_", t), "_2_avg_log2FC")
    )
  } else {
    deg_merge <- deg %>% select(
      ensembl,
      ...
      full_join(deg_merge, by = "ensembl")
  }
}
write.csv(
  deg_merge,
  paste0(sub_output_dir, "correlation_between_replicates_merge_all_plot_data.csv"),
  row.names = FALSE
)
```
...
for (t in treatments) {
  print(t)
  load(paste0(sub_output_dir, "correlation_between_replicates_", t, "_stat_test.RData"))
  print(cor)
  print(cor$p.value)
}
```
...
treatments <- c("Carbo", "Olaparib", "NK", "Control")
for (t in treatments){
  data_tmp <- sig[sig[[paste0(t, "_preR_preS_p")]] < 0.05,] %>%
    select(ensembl, paste0(t, "_preR_preS_p"), ends_with("_DMSO_log2FC"))
  write.csv(
    data_tmp,
    paste0(sub_output_dir, "scatter_plot_", t, "_log2FC_preR_preS_p.csv"),
    row.names = FALSE
  )
}
```
...
```{r}
rm(list = setdiff(ls(), c("output_dir", "red", "blue", "brown", "green", "purple", "red2", "blue2", "brown2", "green2","red2", "blue3", "hrd_color", "hrp_color", "hrd_vector_color")))
sub_output_dir <- paste0(output_dir, "revision4/DEG_within_subclone_A")
if (!dir.exists(sub_output_dir)){
  dir.create(sub_output_dir)
}
treatments <- c("Carbo", "NK", "Olaparib")
...
```{r}
rm(list = setdiff(ls(), c("output_dir", "red", "blue", "brown", "green", "purple", "red2", "blue2", "brown2", "green2","red2", "blue3", "hrd_color", "hrp_color", "hrd_vector_color")))
sub_output_dir <- paste0(output_dir, "data_for_GEO/round_2/")
if (!dir.exists(sub_output_dir)){
  dir.create(sub_output_dir)
}
```
...
write.table(
  scRNA@meta.data,
  paste0(sub_output_dir, "drug_perturbated_sample_cell_info.tsv"),
  sep="\t"
)
write.table(
  scRNA@assays$RNA@counts,
  paste0(sub_output_dir, "drug_perturbated_sample_UMIcounts.tsv"),
  sep="\t"
)
```
