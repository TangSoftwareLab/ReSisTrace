...
```{r}
sig <- read.csv("../data/output/Figure3/Figure3E_original_DEG/Figure3E_logFC_table_for_plots.csv") %>% 
  dplyr::select(ensembl, symbol, Control_log2FC_preR_preS, Control_preR_preS_p, Control_preR_preS_p_adj) %>% 
  right_join(
    read.csv("../data/input/scRNA_seq_for_drug_signature/preR_preS_drug_signature_all.csv"),
    by = c("ensembl", "symbol")
  )
