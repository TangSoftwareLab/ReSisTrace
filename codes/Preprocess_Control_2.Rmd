---
title: "Quality Control for AML samples treated by quizartinib (LT_qu)"
author: "Shuyu Zheng"
date: "`r Sys.Date()`"
output:
  html_notebook:
...
```{r}
bt_sample_path <- "../data/output/cellranger/scLT-ctrl-BT-2/outs/filtered_feature_bc_matrix/"
at_sample_path <- "../data/output/cellranger/scLT-ctrl-AT-2/outs/filtered_feature_bc_matrix/"
bt_label_path <- "../data/output/umi_tools/scLT_ctrl_2/scLT-ctrl-BT-2/adjusted_sequence_count_df.csv"
at_label_path <- "../data/output/umi_tools/scLT_ctrl_2/scLT-ctrl-AT-2/adjusted_sequence_count_df.csv"

output_dir <- "../data/output/scLT_ctrl_2/preprocess/"
...
### 1. Thresholds for before treatment sample
...
```{r}
low.count.bt <- 10000 # Adjust the lower bounder of UMI count here
up.count.bt <- 400000 # Adjust the upper bounder of UMI count here
...
up.feature.bt <- 12500 # Adjust the upper bounder of Feature count here
...
```{r}
low.count.at <- 10000 # Adjust the lower bounder of UMI count here
...
seurat_qc <- data.frame(
  sample = c("before_treatment", "after_treatment"),
  umi_count_lower = c(low.count.bt, low.count.at),
  umi_count_upper = c(up.count.bt, up.count.at),
  gene_count_lower = c(low.feature.bt, low.feature.at),
  gene_count_upper = c(up.feature.bt, up.feature.at),
  mit_percentage = c(up.mt.bt, up.mt.at),
  cell_count_before_qc = c(ncol(bt), ncol(at)),
  cell_count_after_qc = c(ncol(bt_f), ncol(at_f)),
  number_of_genes = c(nrow(bt_f), nrow(at_f)),
  stringsAsFactors = FALSE
)
write.csv(seurat_qc, paste0(output_dir, "/Seurat_qc.csv"), row.names = FALSE)
```
...
```
```{r}
lineage_label_count <- data.frame(
  sample = c("before_treatment", "after_treatment"),
  bam_file = c(
    length(unique(df_lin_b$adjusted_sequence)),
    length(unique(df_lin_a$adjusted_sequence))
...
ggsave(paste0(output_dir, "lineage_labels_counts_per_cell_with_low_quality_cells.png"))
```
...
venn.diagram(
  ...
  filename = paste0(output_dir, "lineage_label_sequences_overlap_without_low_quality_cells.png"),
  output=FALSE,
...
ggplot(data = l_b_count_no_qc, aes(total_count)) +
  ...
  labs(y = "count of unique labels")
  stat_bin(
    aes(y=..count.., label=..count..),
    geom="text", vjust=-.5,
    bins = max(l_b_count_no_qc$total_count)
  )
ggsave(paste0(output_dir, "number_of_labels_with_corresponding_occurrnce_in_bt.png"))
```
...
p <- venn.diagram(
  ...
  filename = paste0(
    output_dir,
    "lineage_label_combination_overlap_with_low_quality_cells.png"
  ),
  output=FALSE,
...
```{r}
lineage_label_count$combo_before_qc <- c(
  length(unique(l_b_fre_merge_filtered$lineage_id)),
  length(unique(l_a_fre_merge_filtered$lineage_id))
)
write.csv(
  lineage_label_count,
  paste0(output_dir, "lineage_label_count_qc.csv"),
  row.names = FALSE
)
```
...
p <- venn.diagram(
  ...
  filename = paste0(
    output_dir,
    "lineage_label_combination_overlap_without_low_quality_cells.png"
  ),
  output=FALSE,
...
```{r}
lineage_label_count$combo_after_qc <- c(
  length(unique(l_b_fre_merge_filtered$lineage_id)),
  length(unique(l_a_fre_merge_filtered$lineage_id))
)
write.csv(
...
* **pre-sensitive**: The cells in BT sample which have the perfect/partial matched cells (sisters) in the AT sample. It can only appear in BT samples.
* **pre-resistant**: The cells in BT sample which does **not** perfect/partial match to any cells in the AT sample. It can only appear in BT samples.
...
VlnPlot(bt_f_p, features = c("nFeature_RNA"),
  ...
  stat_summary(fun = mean, geom='point', size = 25, colour = "white", shape = 95) +
  theme(axis.title.x=element_blank(),
...
# cell groups
tmp <- data.frame(table(merge$drugSens))
names(tmp) <- c("group", "number_of_cells")
tmp <- rbind.data.frame(
  tmp,
  data.frame(
    group = c("BT total", "AT total"),
    number_of_cells = c(ncol(bt_f), ncol(at_f))
  )
)
write.csv(
  tmp,
  paste0(output_dir, "/cell_count_in_drugSens_groups.csv"),
  row.names = FALSE
)
# sister hood in BT
tmp <- data.frame(table(table(merge$sisters)))
names(tmp) <- c("number_of_sisters_per_lineage", "number_of_lineages")
write.csv(
  tmp,
  paste0(output_dir, "/sister_hood_in_BT_sample.csv"),
  row.names = FALSE
)
```
