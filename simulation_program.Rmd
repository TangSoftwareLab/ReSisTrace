...
for (m in metrics) {
  tmp <- plot_table
  tmp[, startsWith(colnames(tmp), m)] <- tmp[, startsWith(colnames(tmp), m)] * 100
  p<- ggplot(
      tmp,
      aes_string(
        x="KillRateTotal",
    ...
    theme(
      ...
      axis.title = element_text(size = 20, color = "black", hjust = 0.5),
  ...
  ggsave(paste0(output_dir, m, "_lineage_count.png"), p, width = 6, height = 4)
  
  p<- ggplot(
      tmp,
      aes_string(
        x="KillRateTotal",
        y=paste0(m, "Cell_mean"),
        group="treatment",
        color="treatment"
      )
    ) + 
  ...
  ggsave(paste0(output_dir, m, "_cell_count.png"), p, width = 6, height = 4)
  
  p<- ggplot(
      tmp,
      aes_string(
        x="KillRateTotal",
        y=paste0(m, "Singleton_mean"),
        group="treatment",
        color="treatment"
      )
    ) + 
  ...
  ggsave(paste0(output_dir, m, "_lineage_count_singleton.png"), p, width = 6, height = 4)
  
  p<- ggplot(
      tmp,
      aes_string(
        x="KillRateTotal",
        y=paste0(m, "CellSingleton_mean"),
        group="treatment",
        color="treatment"
      )
    ) + 
    ...
    ylim(c(0, 100)) +
    labs(
      title = "Cell count (only singleton)",
      x = "Killing rate",
      y = "Negative predictive value",
      color = ""
    )
  ggsave(paste0(output_dir, m, "_cell_count_singleton.png"), p, width = 6, height = 4)
}
```
```{r}
