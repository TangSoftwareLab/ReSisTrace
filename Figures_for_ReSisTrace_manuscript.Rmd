...
ggsave(
  paste0(sub_output_dir, "FigureS1B2_frequence_of_lineage_lable.pdf"),
  width = 5, height = 5
)
```
...
ggsave(
  paste0(sub_output_dir, "Figure3C_COV362_Olaparib.pdf"),
  width = 4.5, height = 4
)
ggsave(
  paste0(sub_output_dir, "Figure3C_COV362_Olaparib.png"),
  width = 6, height = 4
)
```
...
table$sample[table$sample == "COV362-GFP-M"] <- "COV362-control"
table$sample[table$sample == "COV362-B1-GFP-M"] <- "COV362-BRCA1 restored"
table$sample <- factor(table$sample, levels = c("COV362-control", "COV362-BRCA1 restored"))
...
table$sample[table$sample == "Kura-GFP-H"] <- "KURAMOCHI-control"
table$sample[table$sample == "Kura-B2-GFP-H"] <- "KURAMOCHI-BRCA2 restored"
table$sample <- factor(table$sample, levels = c("KURAMOCHI-control", "KURAMOCHI-BRCA2 restored"))
...
for (t in treatments) {
  ...
  for (i in drugs){
    ...
    data_tmp <- data_merge[data_merge[[paste0("boot_", t, "_ctrl_p_value")]] < 0.05, ]
    # preR-preS adjusted growth control
    cor <- cor.test(
    ...
    data_tmp <- data_merge[data_merge[[paste0("perm_", t, "_ctrl_p_value")]] < 0.05, ]
    # preR-preS adjusted growth control
    cor <- cor.test(
      data_tmp[[i]], data_tmp[[paste0(t, "_log2FC_preR_preS_adj")]],
      method = "pearson",
...
for (t in treatments) {
  ...
  for (i in drugs){
    ...
    data_tmp <- data_merge[
      data_merge[[paste0("perm_", t, "_ctrl_p_value")]] < 0.05 &
        data_merge[[paste0("boot_", t, "_ctrl_p_value")]] < 0.05,
      ]
    # preR-preS adjusted growth control
    cor <- cor.test(
      data_tmp[[i]], data_tmp[[paste0(t, "_log2FC_preR_preS_adj")]],
...
for (t in treatments){
  ...
  highlight <- filter(plot_table, drug %in% highlight_drug)
  ...
  p <- ggplot() +
     ...
     labs(x = expression("GWC Connectivity Score"),
          y = expression("-log"[10]~"(P value)"),
